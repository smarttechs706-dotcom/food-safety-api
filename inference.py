...
# ─── Image models ─────────────────────────────────────────────────────────────
xgb_v1 = load_model('best_image_classifier_98pct.pkl')   # 98%
xgb_v2 = load_model('image_classifier_91pct.pkl')        # 91%
...
def advanced_inference(image_path, model='v1'):
    ...
    # MobileNetV2 features (1280-dim) — matches training
    img_tensor = mobilenet_transform(img).unsqueeze(0).to(device)
    with torch.no_grad():
        features = mobilenet(img_tensor)
    final_features = features.cpu().numpy().flatten()  # shape: (1280,)
    ...
    # Anomaly score using pixel variance as proxy
